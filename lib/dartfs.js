...
function dartFs(opts) {
  ...
  return function(input) {
    return ops.execDartVersion().then(function(dartVersion) {
      var waitingOn = [];
      var changed = 0;
      ...
      // Use the configuration in the hash
      for(var o in opts) {
        // fetcherBaseUrl is not relevant for caching.
        if (o != "fetcherBaseUrl") {
          shasum.update("" + opts[o]);
        }
      }
      shasum.update(dartVersion);
